...
class L private constructor() {
    ...
    open class DebugTree : Tree() {
        ...
        override fun log(priority: Int, tag: String?, message: String, t: Throwable?) {
            val stacktrace = Throwable().stackTrace[4]
            val methodName = stacktrace.methodName
            val line = stacktrace.lineNumber
            val file = stacktrace.fileName
            if (message.length < MAX_LOG_LENGTH) {
                val message = "$methodName: $message ($file:$line)"
            ...
            while (i < length) {
                ...
                do {
                    ...
                    val part = methodName + ": " + message.substring(i, end) + " ($file:$line)"
                    if (priority == Log.ASSERT) {
                        Log.wtf(tag, part)
                    } else {
