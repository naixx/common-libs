...
inline fun <reified E : Enum<E>> PrefsDsl.enum(defaultValue: E, key: String? = null): ReadWriteProperty<PrefsDsl, E> =
    enumDelegate(defaultValue, key, E::class, this)

@PublishedApi
internal fun <E : Enum<E>> enumDelegate(defaultValue: E, key: String? = null, c: KClass<E>, dsl: PrefsDsl): ReadWriteProperty<PrefsDsl, E> =
    PrefPrimitiveDelegate(
        key,
        defaultValue,
        { s: String, e: E -> K.enumValueOf(c.java, getString(s, null) ?: defaultValue.name) },
        { s: String, e: E -> putString(s, e.name) }
    )

private class PrefPrimitiveDelegate<T>(
